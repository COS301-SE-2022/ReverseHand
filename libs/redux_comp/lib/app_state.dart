...
class AppState {
  ...
  final bool signUpComplete; // users email
  final List<Advert> adverts; //supposed to be final, ask mike...

  // constructor must only take named parameters
  const AppState(
      {required this.id, required this.username, required this.adverts, required this.signUpComplete});
  ...
  factory AppState.initial() {
    ...
    return const AppState(id: "", username: "", adverts: [], signUpComplete: false);
  }

  factory AppState.mock() {
    return const AppState(id: "1234567890", username: "TestName", adverts: [], signUpComplete: false);
  }

  // easy way to replace store wihtout specifying all paramters
  AppState replace({String? id, String? username, List<Advert>? adverts, bool? signUpComplete}) {
    return AppState(
        id: id ?? this.id,
